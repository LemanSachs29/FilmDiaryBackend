...
class MovieDetailManager {
  ...
  initializeElements() {
    ...
    this.movieTagline = document.getElementById("movie-tagline");
  ...
  async loadMovieDetails() {
    ...
    try {
      ...
      this.showError(
        "No se pudo cargar la información de esta película: " + error.message
      );
    }
  }
  ...
  displayMovieDetails() {
    ...
    // Poster
    this.moviePoster.src = this.getTmdbImageUrl(movie.poster_path);
    this.moviePoster.alt = movie.title;

    // Títulos
    this.movieTitle.textContent = movie.title;
    this.movieOriginalTitle.textContent =
      movie.original_title !== movie.title ? `(${movie.original_title})` : "";

    // Tagline
    this.movieTagline.textContent = movie.tagline || "";

    // Información básica
    this.movieReleaseDate.textContent = this.formatDate(movie.release_date);
    this.movieRuntime.textContent = this.formatRuntime(movie.runtime);
    this.movieRating.textContent = movie.vote_average
    ...
    this.movieLanguage.textContent = this.getLanguageName(
    ...
    this.movieVoteAverage.textContent = movie.vote_average
      ? movie.vote_average.toFixed(1)
      : "-";
    this.movieVoteCount.textContent = movie.voteCount
      ? movie.voteCount.toLocaleString()
      : "-";
    this.moviePopularity.textContent = movie.popularity
  ...
  displayGenres(genres) {
    ...
    const genreHtml = genres
      .map(
        (genre) => `<span class="badge bg-secondary me-1">${genre.name}</span>`
      )
      .join("");
  ...
  handleStarRating(event) {
    ...
    // Solo mostrar la puntuación numérica
    //this.ratingText.textContent = value > 0 ? `${value}/5` : '';
  }
  ...
  updateWatchlistButton(isInWatchlist) {
    if (isInWatchlist) {
      this.addToWatchlistBtn.textContent = "⏰ En Mi Lista";
      this.addToWatchlistBtn.disabled = true;
      this.addToWatchlistBtn.classList.remove("btn-warning");
      this.addToWatchlistBtn.classList.add("btn-secondary");
    } else {
      this.addToWatchlistBtn.textContent = "⏰ Añadir a Mi Lista";
      this.addToWatchlistBtn.disabled = false;
      this.addToWatchlistBtn.classList.remove("btn-secondary");
      this.addToWatchlistBtn.classList.add("btn-warning");
    }
  }
  ...
  getTmdbImageUrl(posterPath) {
    if (!posterPath || posterPath === "null") {
      return "https://placehold.co/400x600/cccccc/666666?text=Sin+Imagen";
    }
    ...
    // ✅ CORRECCIÓN: Asegurar que la URL esté bien formada
