...
class DiaryManager {
    ...
    attachEventListeners() {
        ...
        this.deleteEntryBtn.addEventListener('click', () => this.handleDeleteEntry());
        
        // Star rating en modal
        this.editStarButtons.forEach(btn => {
            btn.addEventListener('click', (e) => this.handleEditStarRating(e));
        });
        this.editStarContainer.addEventListener('dblclick', () => this.resetEditRating());
    }
    ...
    displayDiary() {
        ...
        const tableHtml = filteredData.map(entry => {
    ...
    calculateStats() {
        ...
        const ratingsWithScore = this.diaryData.filter(entry => entry.puntuacion > 0);
        if (ratingsWithScore.length > 0) {
            const average = ratingsWithScore.reduce((sum, entry) => sum + entry.puntuacion, 0) / ratingsWithScore.length;
     ...
     */
    populateYearFilter() {
        const years = [...new Set(this.diaryData.map(entry => 
    ...
    setEditRating(rating) {
        ...
        this.editRatingText.textContent = ratingTexts[rating] || 'Sin puntuación';
    }
    ...
    async handleSaveEdit() {
        ...
        try {
            await api.updateDiaryEntry(this.currentEditEntry.idEntradaPelicula, fechaVisionado, puntuacion);
            ...
            this.showToast('Error al actualizar la entrada: ' + error.message, 'danger');
        } finally {
    ...
    confirmDeleteEntry(entryId) {
        ...
        if (confirm(`¿Estás seguro de que quieres eliminar "${movieTitle}" de tu diario?\n\nEsta acción no se puede deshacer.`)) {
    ...
    async handleDeleteEntry(entryId = null) {
        const targetId = entryId || (this.currentEditEntry ? this.currentEditEntry.idEntradaPelicula : null);
        if (!targetId) return;
        ...
        try {
            ...
            this.showToast('Error al eliminar la entrada: ' + error.message, 'danger');
        } finally {
